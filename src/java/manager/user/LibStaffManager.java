...
public class LibStaffManager extends AbstractUserManager {
	...
	public boolean add(Patron patron) {
		...
		try {
			...
			preparedStatement.setString(1, libManager.getUserName());
	...
	public List<Patron> gets() {
		...
		try{
			...
			while(resultSet.next()){
				...
				libManager.setUserName(resultSet.getString(PATRON_USER_NAME));
				libManager.setPassword(resultSet.getString(PATRON_PASSWORD));
				...
				libManager.setFullName(resultSet.getString(PATRON_FULL_NAME));
	...
	public boolean update(Patron patron) {
		...
		*/
		...
		String sql1 = "UPDATE " + LIB_MANAGER_TABLE_NAME + " SET " +
			...
			" WHERE "+ PATRON_USER_NAME + "='"+libManager.getUserName()+"';";
